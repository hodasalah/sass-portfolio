...
$window.on("load", function () {
    ...
    // typed plugin
    if ($("header .slider-content .typed-word").length) {
        new Typed("header .slider-content .typed-word", {
            strings: ["Websites", "React Apps", "Wordpress"],
            loop: true,
            startDelay: 300,
            typeSpeed: 60,
            backSpeed: 60,
...
$(function () {
    ...
    $document.on("click", function (e) {
        if (
            ...
            $(e.target).closest(
                $("header nav .navigation .button-when-menu-is-open")
            ).length
        ) {
    ...
    $window.on("scroll", function () {
        let sec_position = $(this).scrollTop();

        $("body > .section").each(function () {
            let top = $(this).offset().top - 100,
                bottom = top + $(this).outerHeight();

            if (sec_position >= top && sec_position <= bottom) {
                $(
